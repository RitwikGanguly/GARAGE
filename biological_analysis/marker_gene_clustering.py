...
GAN/VAE/LSH-GAN: fixed Leiden resolution = 1.0.
GARAGE: dataset-specific resolution sweep (tuned per dataset).

Pseudo-labelling strategy:
  Primary: NearestCentroid on real data → predict on gen.
  Fallback (when NC produces < 2 classes): cluster gen with Leiden,
  assign each cluster to majority cell type among its k-NN real neighbours.
...
def get_pseudo_labels(gen_filt, real_filt, real_labels_enc):
    ...
    # Fallback: 3-NN majority vote — each gen cell gets the majority label
    # among its 3 nearest real neighbours.  Produces diverse pseudo-labels
    # even when gen cells cluster toward a narrow region of real space.
...
def evaluate_baseline(gen_filt, real_filt, real_labels_enc,
                      n_pcs=N_PCS, n_neighbors=N_NEIGHBORS):
    return compute_ari_nmi(gen_filt, real_filt, real_labels_enc,
                           resolution=1.0, n_pcs=n_pcs, n_neighbors=n_neighbors)


def evaluate_sweep(gen_filt, real_filt, real_labels_enc,
                   res_range, n_pcs=N_PCS, n_neighbors=N_NEIGHBORS):
...
def evaluate_real_reference(real_filt, real_labels_enc,
    ...
    for res in np.arange(0.1, 3.01, 0.1):
        try:
            sc.tl.leiden(adata, resolution=round(res, 2), random_state=42)
            y_pred = adata.obs["leiden"].astype(str).astype(int).to_numpy()
...
def evaluate_clustering(gen_data, real_data, real_labels, marker_idx,
    ...
    elif method_name == "GARAGE":
        return evaluate_sweep(gen_filt, real_filt, real_labels_enc,
                              GARAGE_RES_RANGES[dataset], n_pcs, n_neighbors)
    else:
        return evaluate_baseline(gen_filt, real_filt, real_labels_enc,
                                 n_pcs, n_neighbors)
...
def main():
    ...
    for dataset in DATASET_CONFIG:
        ...
        r_ari, r_nmi = evaluate_clustering(
            real, real, labels_raw, marker_idx, "Real (marker ref.)", dataset)
        ...
        for method_name, dir_name, prefix, suffix in METHODS:
            try:
                ...
                ari, nmi = evaluate_clustering(
                    gen, real, labels_raw, marker_idx, method_name, dataset)
    ...
    df_markers.to_csv(os.path.join(OUT_DIR, "rev5_marker_genes.csv"), index=False)

    df_perf = pd.DataFrame(perf_rows, columns=["Dataset", "Method", "ARI", "NMI"])
    df_perf.to_csv(os.path.join(OUT_DIR, "rev5_clustering_performance.csv"), index=False)

    print(f"\n{'='*80}")
    print(" TABLE Sx : Fixed marker-gene clustering performance")
    print(" GAN/VAE/LSH-GAN: fixed res=1.0  |  GARAGE/Real: tuned res")
    print(f"{'='*80}")
