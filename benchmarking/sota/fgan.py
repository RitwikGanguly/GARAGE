#!/usr/bin/env python
"""
f-GAN (f-divergence GAN) for scRNA-seq data generation (PyTorch).
===================================================================
Uses a Fisher-ratio objective with a second-moment constraint penalty
rather than the standard BCE loss.  Generator: 128-LeakyReLU-128-linear.
Discriminator: 128-LeakyReLU-128-linear → T(x).  Adam, 2,001 epochs.

Usage:  python -m benchmarking.sota.fgan --dataset muraro
Output: data/gen_data/fgan/<dataset>_fgan_generated_mixdata_iter{iter}.csv
"""
import argparse
import os
import numpy as np
...
EPOCHS = 2001
LR = 5e-4
LAMBDA = 10.0  # constraint penalty weight
...
class Generator(nn.Module):
    def __init__(self, zdim, out_dim):
        ...
        self.net = nn.Sequential(
            nn.Linear(zdim, 128), nn.LeakyReLU(),
...
def train(real, n_features, seed=42):
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(0, n, BATCH_SIZE):
            ...
            fake_g = G(z)
            T_fake_g = D(fake_g)
            fr_g = fisher_ratio(D(batch_t), T_fake_g)
            g_loss = -fr_g
            optG.zero_grad()
            g_loss.backward()
            optG.step()
...
def main():
    parser = argparse.ArgumentParser(description="f-GAN benchmark")
    ...
    print(f"fGAN — {args.dataset.upper()}  |  cells={n_sample}  genes={n_features}")

    G = train(real, n_features, seed=args.seed)

    out_dir = args.out or os.path.join(DATA_DIR, "gen_data", "fgan")
    os.makedirs(out_dir, exist_ok=True)

    batch_sizes = (np.arange(0.25, 1.75, 0.25) * n_sample).astype(int)
    for i, n_gen in enumerate(batch_sizes):
        syn = generate(G, n_gen)
        fname = f"{args.dataset.lower()}_fgan_generated_mixdata_iter{i}.csv"
